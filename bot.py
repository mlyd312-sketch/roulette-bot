...
def add_group_to_db(chat_id, chat_title, owner_id):
    ...
    if not row:
        cursor.execute("""
            INSERT INTO groups
            (chat_id, chat_title, owner_id, is_active, sub_channel, add_date, added_by)
            VALUES (?, ?, ?, 0, "", ?, ?)
...
def get_group_info(chat_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT chat_title, is_active, sub_channel, add_date FROM groups WHERE chat_id = ?",
        (chat_id,)
    )
    row = cursor.fetchone()
    conn.close()
...
def update_group_channel(chat_id, channel, added_by=0):
    ...
    cursor.execute("""
        UPDATE groups
        SET sub_channel = ?, is_active = 1, add_date = ?, added_by = ?
        WHERE chat_id = ?
...
def main_menu(user=None):
    ...
    markup.row(
        InlineKeyboardButton("شراء بوت ↗", url="https://t.me/u_8_y"),
        InlineKeyboardButton("المطور ↗", url="https://t.me/u_8_y")
    )
...
def send_welcome(message):
    ...
    text = (
        f"أهلاً {user_name}
"
        "• لاستخدام البوت يجب عليك التالي :-

"
        "- أضف البوت للمجموعة ورفعه مشرفاً ثم أرسل ( تفعيل ) ثم تابع التعليمات التي يرسلها البوت.

"
        "- لإيقاف البوت ارسل : ( تعطيل ) في المجموعة."
    )
    bot.send_message(message.chat.id, text, reply_markup=main_menu(message.from_user))

@bot.message_handler(
    func=lambda message: message.chat.type in ["group", "supergroup"],
    content_types=["text", "audio", "document", "photo", "sticker", "video", "video_note", "voice", "forward"]
)
            ...
            if isinstance(state_data, int):
                ...
                if channel_target:
                    ...
                    try:
                        ...
                        bot.reply_to(message, "<b>~ تم حفظ الايدي بنجاح</b>")
                        return
                    except Exception:
                        bot.reply_to(
                            message,
...
~ قم برفع البوت مشرف في قناتك او مجموعتك العامة وققم بتوجية منشور من القناة او معرف القناة او المجموعة العامة"
            )
            return
        ...
        if text_content == "تعطيل":
            ...
            conn.commit()
            conn.close()
            bot.reply_to(message, "<b>تم تعطيل البوت وإيقاف الاشتراك الإجباري لهذه المجموعة بنجاح</b>")
            return
...
"
                    "يجب عليك الاشتراك لارسال الرسائل في القناة التالية:
"
                    f"👇 <b>{channel_title}</b>
"
    ...
    if user_id in user_states:
        ...
        if state_data == "waiting_bot_sub_channel":
            ...
            try:
                ...
                bot.reply_to(
                    message,
                    f"<b>✅ تم ضبط قناة الاشتراك الإجباري للبوت بنجاح: {channel}</b>",
                    reply_markup=markup
                )
            except Exception:
                markup = InlineKeyboardMarkup()
                ...
                bot.reply_to(
                    message,
                    "❌ فشل تعيين القناة. تأكد من أن البوت مشرف فيها!",
                    reply_markup=markup
                )
...
"
        ...
        text = (
            "📢 أرسل الآن يوزر القناة (مثلاً @ChannelName) أو قم بتوجيه "
