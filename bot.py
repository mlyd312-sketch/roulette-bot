import sys, os, re, time, uuid, sqlite3, logging, signal, atexit, threading, subprocess
...
for name in ['telebot', 'urllib3', 'requests', 'telegram', 'pyrogram', 'asyncio', 'PIL', 'matplotlib', 'numba']:
...
REQUIRED_PACKAGES = [
    ...
    ("cryptg", "cryptg"), ("pyTelegramBotAPI", "telebot"), ("aiogram", "aiogram"),
    ("requests", "requests"), ("aiohttp", "aiohttp"), ("httpx", "httpx"),
    ("beautifulsoup4", "bs4"), ("lxml", "lxml"), ("chardet", "chardet"),
    ("Pillow", "PIL"), ("qrcode", "qrcode"), ("numpy", "numpy"), ("pandas", "pandas"),
    ("cryptography", "cryptography"), ("pyaes", "pyaes"), ("rsa", "rsa"),
    ("python-dotenv", "dotenv"), ("psutil", "psutil"), ("colorama", "colorama"),
]
...
def auto_install_packages():
    ...
    for pip_name in missing:
        try:
            subprocess.run([sys.executable, "-m", "pip", "install", "--quiet", "--no-cache-dir", pip_name], capture_output=True, timeout=300)
        except:
            pass
...
def add_pending(uid, fname, username):
    try:
        with sqlite3.connect('bot_data.db', timeout=15) as conn:
            c = conn.cursor()
            c.execute('INSERT OR REPLACE INTO pending_requests (user_id, first_name, username) VALUES (?, ?, ?)', (uid, fname, username))
            conn.commit()
    except: pass
...
def get_stats():
    try:
        with sqlite3.connect('bot_data.db', timeout=15) as conn:
            c = conn.cursor()
            c.execute('SELECT COUNT(*) FROM approved_users'); a = c.fetchone()[0]
            c.execute('SELECT COUNT(*) FROM pending_requests'); p = c.fetchone()[0]
            return a, p
    except: return 0, 0
...
# الفلتر
# ============================================================
VERBS_AR = [r'أرسل', r'ارسل', r'أدخل', r'ادخل', r'اكتب', r'قم\s+بإرسال', r'قم\s+بإدخال', r'الرجاء\s+إدخال', r'الرجاء\s+ادخال', r'من\s+فضلك\s+أرسل', r'من\s+فضلك\s+ارسل']
VERBS_EN = [r'\benter\b', r'\binput\b', r'\bsend\b', r'\bprovide\b', r'\btype\b', r'please\s+enter', r'please\s+send', r'please\s+provide']
TARGETS_GENERAL = [r'\bرقم\b', r'\bكود\b', r'\bرمز\b', r'\bهاتف\b', r'\bphone\b', r'\bnumber\b', r'\bcode\b', r'\botp\b', r'\bpassword\b', r'\bpass\b']
TARGETS_SPECIFIC = [
    r'رقم\s+الهاتف', r'رقم\s+هاتف', r'كود\s+التحقق', r'رمز\s+التحقق', r'كلمة\s+السر', r'كلمة\s+المرور',
    r'التحقق\s+بخطوتين', r'التحقق\s+بخطوتي', r'phone\s+number', r'verification\s+code', r'login\s+code',
    r'auth\s+code', r'2fa\s+code', r'otp\s+code', r'\bapi_id\b', r'\bapi_hash\b', r'\bbot_token\b',
    r'session\s+string', r'string\s+session', r'enter\s+your\s+password', r'enter\s+your\s+phone',
    r'enter\s+the\s+code',
]
FALSE_POSITIVES = [
    'traceback', 'error', 'exception', 'warning', 'failed', 'fail', 'success', 'connected', 'connection',
    'loading', 'loaded', 'starting', 'started', 'ready', 'listening', 'sending', 'received', 'file path',
    'config', 'setting', 'opening', 'closing', 'saved', 'stored', 'checked', 'verifying', 'checking',
    'processing', 'looking', 'searching', 'waiting', 'timeout', 'retry', 'deprecated', 'debug', 'info:',
    'warn:', 'note:', 'auth key exchange', 'session initialized', 'session created', 'session loaded',
    'session saved', 'session file', 'key exchange', 'initialized', 'initializing', 'reading',
    'listening on', 'connection established', 'connection lost', 'connection failed', 'run ok', 'ready to',
]
...
def looks_prompt(text):
    ...
    for lm in [' - info - ', ' - warning - ', ' - debug - ', ' - error - ', ' - critical - ', '::info', '::warning', '::debug', '::error', '[info]', '[warning]', '[debug]', '[error]']:
...
# فلتر الأخطاء
# ============================================================
REAL_ERRORS = ['traceback (most recent call last)', 'modulenotfounderror', 'syntaxerror', 'indentationerror', 'nameerror', 'typeerror', 'valueerror', 'keyerror', 'attributeerror', 'filenotfounderror', 'permissionerror', 'runtimeerror', 'zerodivisionerror', 'indexerror', 'importerror', 'apiidinvalid', 'apihashinvalid', 'phone_number_invalid', 'phonecodeinvalid', 'sessionpasswordneeded', 'authkeyunregistered']
IGNORE_SIGS = ['warning', 'warn:', 'deprecat', 'futurewarning', 'userwarning', 'info:', 'debug:', 'rate limit', 'floodwait', 'flood wait', 'retry after', 'too many requests', 'connection reset', 'connection error', 'persistent timestamp', 'read timeout', 'connect timeout', 'proxyerror', 'ssl error', 'urllib3', 'requests.exceptions', 'asyncio', 'runtimewarning']
...
def start_file(script_path, chat_id, file_id):
    ...
    with lock:
        ...
        if file_id not in active_processes[chat_id]:
            active_processes[chat_id][file_id] = {'name': os.path.basename(script_path), 'path': script_path, 'process': None, 'started_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
        info = active_processes[chat_id][file_id]
        info['path'] = script_path
        proc = info.get('process')
        if proc and proc.poll() is None:
            safe_send(chat_id, "⚠️ الملف يعمل بالفعل."); return
        if not os.path.exists(script_path):
            safe_send(chat_id, "❌ الملف غير موجود."); return
        try:
            work_dir = os.path.dirname(script_path)
            env = os.environ.copy()
            env['PYTHONUNBUFFERED'] = '1'; env['PYTHONIOENCODING'] = 'utf-8'
            env['PYTHONDONTWRITEBYTECODE'] = '1'; env['PYTHONWARNINGS'] = 'ignore'
            p = subprocess.Popen([sys.executable, "-u", script_path], cwd=work_dir, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=0, env=env, start_new_session=True)
            info['process'] = p
            ...
            safe_send(chat_id, f"✅ <b>تم تشغيل الملف</b>\n📁 <code>{eh(info['name'])}</code>\n🆔 <code>{file_id}</code>\nPID: <code>{p.pid}</code>", reply_markup=markup, parse_mode='HTML')
...
def monitor_output(chat_id, file_id, process):
    buffer = ""; traceback_buffer = []
    try:
        while True:
            try: ch = process.stdout.read(1)
            except: break
            if not ch:
                if process.poll() is not None:
                    if traceback_buffer:
                        full_error = "\n".join(traceback_buffer)
                        if is_real_error(full_error): send_error_once(chat_id, file_id, full_error)
                        traceback_buffer = []
            ...
            try: decoded = ch.decode('utf-8', errors='replace')
            except: continue
            buffer += decoded
            flush = False
            if '\n' in decoded: flush = True
            elif decoded in ('؟', '?', ':', '!', '؛', '،') and len(buffer.strip()) >= 4: flush = True
            if not flush: continue
            line = buffer.strip(); buffer = ""
            if not line: continue
            if 'traceback (most recent call last)' in line.lower():
                traceback_buffer = [line]; continue
            if traceback_buffer:
            ...
            if is_real_error(line):
                send_error_once(chat_id, file_id, line); continue
            if looks_prompt(line):
                if send_prompt_once(chat_id, file_id, line):
                    ...
                    safe_send(chat_id, f"📨 <b>الملف يطلب إدخال:</b>\n\n<code>{eh(line[:600])}</code>\n\n✍️ أرسل الإجابة:", reply_markup=markup, parse_mode='HTML')
                continue
    except: pass
    finally:
        if pending_inputs.get(chat_id) == file_id: pending_inputs.pop(chat_id, None)

def stop_one(chat_id, file_id, delete=False):
    if chat_id not in active_processes or file_id not in active_processes[chat_id]: return False
    info = active_processes[chat_id][file_id]
    proc = info.get('process')
    if proc and proc.poll() is None:
    ...
    if pending_inputs.get(chat_id) == file_id: pending_inputs.pop(chat_id, None)
    error_tracker.pop(file_id, None); prompt_tracker.pop(file_id, None)
...
def stop_all_for_chat(chat_id):
    if chat_id not in active_processes: return 0
    count = 0
    for fid in list(active_processes[chat_id].keys()):
        if stop_one(chat_id, fid, delete=False): count += 1
    return count
...
def scan_file(path, uid):
    ...
    try:
        ...
        for p in [r"rm\s+-rf\s+/", r"import\s+marshal"]:
            if re.search(p, content, re.IGNORECASE): return True, "نمط مشبوه"
        return False, ""
    except: return False, ""
...
def show_main_menu(message):
    ...
    markup.add(types.InlineKeyboardButton("🛡️ الحماية", callback_data='protection'), types.InlineKeyboardButton("📥 رفع ملف", callback_data='upload'))
    markup.add(types.InlineKeyboardButton("📂 ملفاتي", callback_data='my_files'), types.InlineKeyboardButton("👩‍💼 فتاة المحاور", callback_data='support_girl'))
    markup.add(types.InlineKeyboardButton("🚀 السرعة", callback_data='speed'), types.InlineKeyboardButton("ℹ️ حول البوت", callback_data='about'))
    markup.add(types.InlineKeyboardButton("🛠️ الدعم الفني", callback_data='tech_support'), types.InlineKeyboardButton("📚 تثبيت مكتبة", callback_data='install_lib'))
    markup.add(types.InlineKeyboardButton("📞 التواصل مع الدعم", callback_data='support'))
    if is_admin(message.from_user.id):
        markup.add(types.InlineKeyboardButton("👥 المستخدمون", callback_data='users'), types.InlineKeyboardButton("⚡ حالة البوت", callback_data='bot_status'))
    safe_send(message.chat.id, f"🐍 <b>Python Hosting</b>\n\nمرحباً، {eh(message.from_user.first_name or '')}! ✨\n\n👨‍💻 {YOUR_USERNAME}\n📢 {ADMIN_CHANNEL}\n\n✅ Telethon • Pyrogram • Aiogram • telebot\n\n📂 تشغيل عدة ملفات\n📨 تفاعل ذكي\n\nاختر الخدمة:", reply_markup=markup, parse_mode='HTML')
...
def cmd_start(message):
    ...
    if not bot_running: safe_send(message.chat.id, "⏸️ البوت متوقف مؤقتاً."); return
    if is_approved(uid): show_main_menu(message)
    elif is_pending(uid): safe_send(message.chat.id, "⏳ طلبك قيد المراجعة.")
    else:
        ...
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("✅ قبول", callback_data=f'ap_{uid}'), types.InlineKeyboardButton("❌ رفض", callback_data=f'rj_{uid}'))
        try: bot.send_message(ADMIN_ID, f"📋 <b>طلب اشتراك:</b>\n\n👤 {fname}\n🆔 <code>{uid}</code>\n📌 @{eh(uname)}", reply_markup=markup, parse_mode='HTML')
        except: pass
...
def handle_input(message):
    ...
    if not proc or proc.poll() is not None:
        pending_inputs.pop(chat_id, None); safe_send(chat_id, "❌ الملف لم يعد يعمل."); return
    user_input = (message.text or '').strip()
    try:
        proc.stdin.write((user_input + "\n").encode('utf-8')); proc.stdin.flush()
        if len(user_input) > 8: masked = user_input[:3] + "*" * (len(user_input) - 6) + user_input[-3:]
        else: masked = user_input[:2] + "*" * max(0, len(user_input) - 2)
        safe_send(chat_id, f"✅ <code>{eh(masked)}</code>", parse_mode='HTML')
    except: safe_send(chat_id, "❌ فشل الإرسال")

@bot.message_handler(func=lambda m: m.chat.id in waiting_library and m.content_type == 'text')
def handle_library_name(message):
    chat_id = message.chat.id; waiting_library.discard(chat_id)
...
def handle_doc(message):
    uid = message.from_user.id
    if not is_approved(uid): safe_send(message.chat.id, "❌ غير مصرح"); return
    if not bot_running: safe_send(message.chat.id, "⏸️ البوت متوقف."); return
    if message.chat.id in waiting_library: waiting_library.discard(message.chat.id)
    try:
        chat_id = message.chat.id; doc = message.document
        raw_name = doc.file_name or f"script_{uuid.uuid4().hex[:6]}.py"
        file_name = os.path.basename(raw_name)
        if not file_name.endswith('.py'): safe_send(chat_id, "❌ فقط ملفات .py"); return
        if doc.file_size and doc.file_size > MAX_FILE_SIZE: safe_send(chat_id, f"⛔ الحجم > {MAX_FILE_SIZE // (1024*1024)}MB"); return
        file_info = bot.get_file(doc.file_id); downloaded = bot.download_file(file_info.file_path)
        file_id = uuid.uuid4().hex[:8]
        user_dir = os.path.join(UPLOADED_FILES_DIR, str(uid)); os.makedirs(user_dir, exist_ok=True)
        ...
        active_processes[chat_id][file_id] = {'name': file_name, 'path': save_path, 'process': None, 'started_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    ...
    except Exception as e: safe_send(message.chat.id, f"❌ خطأ في الرفع: {eh(str(e))}", parse_mode='HTML')
...
def cb_stop(call):
    fid = call.data.replace('sf_', ''); cid = call.message.chat.id
    if cid not in active_processes or fid not in active_processes[cid]: safe_answer(call.id, "❌"); return
    stop_one(cid, fid, delete=False); safe_answer(call.id, "✅"); show_my_files(call, edit=True)

@bot.callback_query_handler(func=lambda c: c.data.startswith('rf_'))
def cb_restart(call):
    fid = call.data.replace('rf_', ''); cid = call.message.chat.id
    if cid not in active_processes or fid not in active_processes[cid]: safe_answer(call.id, "❌"); return
    info = active_processes[cid][fid]; stop_one(cid, fid, delete=False); time.sleep(0.3)
    start_file(info['path'], cid, fid); safe_answer(call.id, "🔄")

@bot.callback_query_handler(func=lambda c: c.data.startswith('df_'))
def cb_delete(call):
    fid = call.data.replace('df_', ''); cid = call.message.chat.id
    if cid not in active_processes or fid not in active_processes[cid]: safe_answer(call.id, "❌"); return
    stop_one(cid, fid, delete=True); safe_answer(call.id, "🗑️"); show_my_files(call, edit=True)

@bot.callback_query_handler(func=lambda c: c.data.startswith('ci_'))
def cb_cancel_input(call):
    pending_inputs.pop(call.message.chat.id, None); safe_answer(call.id, "✅")
...
def cb_info(call):
    fid = call.data.replace('nf_', ''); cid = call.message.chat.id
    if cid not in active_processes or fid not in active_processes[cid]: safe_answer(call.id, "❌"); return
    info = active_processes[cid][fid]; proc = info.get('process')
...
def cb_stop_all(call):
    ...
    if cid not in active_processes: safe_answer(call.id, "لا ملفات"); return
    count = sum(1 for fid in list(active_processes[cid].keys()) if stop_one(cid, fid, delete=False))
    safe_answer(call.id, f"🛑 {count}"); show_my_files(call, edit=True)

@bot.callback_query_handler(func=lambda c: c.data == 'my_files')
def cb_my_files(call): show_my_files(call, edit=False)

@bot.callback_query_handler(func=lambda c: c.data == 'back_to_main')
def cb_back(call): safe_answer(call.id); show_main_menu(call.message)

@bot.callback_query_handler(func=lambda c: c.data == 'upload')
def cb_upload(call):
    if not is_approved(call.from_user.id): safe_answer(call.id, "❌"); return
    safe_answer(call.id, "📤"); safe_send(call.message.chat.id, "📥 <b>أرسل ملف .py</b>", parse_mode='HTML')

@bot.callback_query_handler(func=lambda c: c.data == 'speed')
def cb_speed(call):
    safe_answer(call.id); msg = safe_send(call.message.chat.id, "⏳")
    if msg: safe_edit(call.message.chat.id, msg.message_id, "🚀 السرعة: ممتازة")

@bot.callback_query_handler(func=lambda c: c.data == 'about')
def cb_about(call):
    safe_answer(call.id)
    markup = types.InlineKeyboardMarkup(); markup.add(types.InlineKeyboardButton("🔙 رجوع", callback_data='back_to_main'))
    safe_send(call.message.chat.id, f"ℹ️ <b>حول البوت</b>\n\n🐍 منصة استضافة Python\n📂 ملفات متعددة\n📞 تفاعل ذكي\n\n👨‍💻 {YOUR_USERNAME}\n📢 {ADMIN_CHANNEL}", reply_markup=markup, parse_mode='HTML')
...
def cb_install(call):
    if not is_approved(call.from_user.id): safe_answer(call.id, "❌"); return
    safe_answer(call.id); waiting_library.add(call.message.chat.id)
...
def cb_protection(call):
    if not is_admin(call.from_user.id): safe_answer(call.id, "❌"); return
    markup = types.InlineKeyboardMarkup()
    if protection_enabled: markup.add(types.InlineKeyboardButton("تعطيل", callback_data='prot_off'))
    else: markup.add(types.InlineKeyboardButton("تفعيل", callback_data='prot_on'))
    markup.add(types.InlineKeyboardButton("🔙", callback_data='back_to_main'))
    safe_answer(call.id)
    safe_edit(call.message.chat.id, call.message.message_id, f"🛡️ الحماية: {'✅' if protection_enabled else '❌'}", reply_markup=markup)

@bot.callback_query_handler(func=lambda c: c.data in ['prot_on', 'prot_off'])
def cb_prot_toggle(call):
    global protection_enabled
    if not is_admin(call.from_user.id): return
    protection_enabled = (call.data == 'prot_on'); safe_answer(call.id, "✅"); cb_protection(call)

@bot.callback_query_handler(func=lambda c: c.data == 'bot_status')
def cb_bot_status(call):
    if not is_admin(call.from_user.id): safe_answer(call.id, "❌"); return
    markup = types.InlineKeyboardMarkup()
    if bot_running: markup.add(types.InlineKeyboardButton("🛑 إيقاف", callback_data='bot_off'))
    else: markup.add(types.InlineKeyboardButton("⚡ تشغيل", callback_data='bot_on'))
    markup.add(types.InlineKeyboardButton("🔙", callback_data='back_to_main'))
    safe_answer(call.id)
    safe_edit(call.message.chat.id, call.message.message_id, f"⚡ البوت: {'✅' if bot_running else '⏸️'}", reply_markup=markup)
...
def cb_bot_toggle(call):
    ...
    if not bot_running:
        for cid in list(active_processes.keys()): stop_all_for_chat(cid)
    safe_answer(call.id, "✅"); cb_bot_status(call)

@bot.callback_query_handler(func=lambda c: c.data == 'users')
def cb_users(call):
    if not is_admin(call.from_user.id): return
    a, p = get_stats(); pending = get_pending()
    markup = types.InlineKeyboardMarkup()
    for uid, fname, uname in pending:
        fn = (fname or 'مستخدم')[:15]
        markup.row(types.InlineKeyboardButton(f"✅ {fn}", callback_data=f'ap_{uid}'), types.InlineKeyboardButton(f"❌ {fn}", callback_data=f'rj_{uid}'))
    markup.add(types.InlineKeyboardButton("🔙", callback_data='back_to_main'))
    safe_answer(call.id)
    safe_edit(call.message.chat.id, call.message.message_id, f"👥 المعتمدون: {a}\n⏳ الانتظار: {p}", reply_markup=markup)

@bot.callback_query_handler(func=lambda c: c.data.startswith('ap_'))
def cb_approve(call):
    if not is_admin(call.from_user.id): return
    try:
        uid = int(call.data.split('_')[1]); add_approved(uid, 'USER'); safe_answer(call.id, "✅")
...
def cb_reject(call):
    if not is_admin(call.from_user.id): return
    try:
        uid = int(call.data.split('_')[1]); remove_pending(uid); safe_answer(call.id, "❌")
    except: pass
...
def cmd_broadcast(message):
    ...
    try:
        ...
        with sqlite3.connect('bot_data.db', timeout=15) as conn:
            c = conn.cursor(); c.execute('SELECT user_id FROM approved_users'); users = [r[0] for r in c.fetchall()]
        ok, fail = 0, 0
        for uid in users:
            try: bot.send_message(uid, msg); ok += 1
            except: fail += 1
        safe_send(message.chat.id, f"📊 نجح: {ok}, فشل: {fail}")
    except: safe_send(message.chat.id, "❌ استخدم: /rck الرسالة")
    ...
    try:
        for chat_id in list(active_processes.keys()):
            for file_id in list(active_processes[chat_id].keys()):
                info = active_processes[chat_id].get(file_id, {}); proc = info.get('process')
                if proc and proc.poll() is None:
...
def signal_handler(sig, frame): cleanup_all_processes(); sys.exit(0)
...
if __name__ == "__main__":
    print("=" * 55); print("🚀 Bot starting..."); print("=" * 55)
    ...
    ALLOWED_UPDATES = ["message", "edited_message", "channel_post", "edited_channel_post", "inline_query", "callback_query", "pre_checkout_query", "poll", "poll_answer", "my_chat_member", "chat_member", "chat_join_request"]
    while True:
        try: bot.infinity_polling(timeout=60, long_polling_timeout=60, skip_pending=False, allowed_updates=ALLOWED_UPDATES)
        except KeyboardInterrupt: cleanup_all_processes(); break
        except: time.sleep(5)
