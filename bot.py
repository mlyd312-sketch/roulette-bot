...
# ======= كلمات كشف طلبات الإدخال ======= #
PROMPT_KEYWORDS = [
    'يطلب', 'أرسل', 'ادخل', 'أدخل', 'اكتب', 'رقم', 'كود', 'رمز',
    'كلمة السر', 'كلمة سر', 'هاتف', 'الهاتف', 'تحقق', 'التحقق',
    'توثيق', 'التوثيق', '2fa', 'otp', 'otp!', 'كود التحقق',
    'enter', 'input', 'phone', 'number', 'code', 'password',
    'verify', 'please', 'send', 'otp:', '2fa:', 'tel', 'token',
]
    ...
    try:
        ...
        if protection_level == "low":
            ...
            dangerous_patterns = [
                ...
                r"import\s+base64", r"eval\s*\(", r"exec\s*\(",
                r"shutil\.make_archive", r"subprocess\.call",
            ...
            dangerous_patterns = [
                r"rm\s+-rf\s+[\'\"]?/", r"import\s+marshal", r"import\s+zlib",
                r"import\s+base64", r"eval\s*\(", r"exec\s*\(",
                r"shutil\.make_archive", r"subprocess", r"os\.system",
                r"os\.popen", r"sys\.exit", r"open\s*\(\s*['\"]/etc",
                r"open\s*\(\s*['\"]/root", r"__import__\s*\(",
                r"getattr\s*\(", r"setattr\s*\(", r"globals\s*\(",
                r"locals\s*\(", r"compile\s*\(",
            ]
...
def _ask_user_for_input(chat_id, script_name, prompt_text, process):
    ...
    text = (
        f'{ce("bell")} <b>الملف يطلب إدخال بيانات</b>\n\n'
        f'{ce("python")} الملف: <code>{script_name}</code>\n\n'
        f'{ce("settings")} <b>الطلب:</b>\n'
        f'<code>{clean_prompt}</code>\n\n'
        f'{ce("pencil")} <b>أرسل الإجابة الآن كرسالة نصية عادية.</b>'
    )
    try:
            ...
            if r:
                ...
                if _is_prompt_text(text):
                    ...
                    if key not in asked_prompts:
                        asked_prompts.add(key)
                        _ask_user_for_input(chat_id, script_name, text, process)
                        buffer = b""
                        continue
                    ...
                    if current_time >= expiry:
                        ...
                        if user_id in bot_scripts and 'processes' in bot_scripts[user_id]:
                            ...
                            if process and process.poll() is None:
                                ...
                                try:
                                    markup = types.InlineKeyboardMarkup()
                                    approve_btn = btn("الموافقة على التجديد",
                                                      callback_data=f'renewapprove_{user_id}_{file_name}',
                                                      emoji_key='check', style="success")
                                    reject_btn = btn("رفض التجديد",
                                                     callback_data=f'renewreject_{user_id}_{file_name}',
                                                     emoji_key='cross', style="danger")
...
def handle_process_input(message):
    ...
    if chat_id not in process_stdin_pending:
        return

    pending = process_stdin_pending[chat_id]
    ...
    try:
        if process.poll() is not None:
            send_with_emoji(chat_id, "⚠️ العملية توقفت بالفعل.", E['warning'])
            del process_stdin_pending[chat_id]
            asked_prompts.discard((chat_id, file_name))
            return

        if process.stdin is None:
            send_with_emoji(chat_id, "❌ لا يمكن الإرسال للملف.", E['cross'])
            del process_stdin_pending[chat_id]
            return
        ...
        send_with_emoji(
            chat_id,
            f"✅ تم إرسال الإجابة إلى <code>{file_name}</code>",
            E['check']
        )
    except Exception as e:
        send_with_emoji(chat_id, f"❌ فشل في إرسال الإجابة: {e}", E['cross'])
        try:
            del process_stdin_pending[chat_id]
            asked_prompts.discard((chat_id, file_name))
        except:
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('cancelinput_'))
def cancel_input_callback(call):
    try:
        parts = call.data.replace('cancelinput_', '').split('_', 1)
        chat_id = int(parts[0])
        file_name = parts[1]

        if chat_id in process_stdin_pending and process_stdin_pending[chat_id]['file_name'] == file_name:
            process = process_stdin_pending[chat_id]['process']
            try:
        ...
        try:
            bot.edit_message_text(
                f"❌ تم إلغاء طلب الإدخال للملف <code>{file_name}</code>",
...
def handle_file(message):
    ...
    try:
        ...
        markup = types.InlineKeyboardMarkup()
        stop_button = btn(f"إيقاف {bot_script_name}",
                          callback_data=f'stopfile_{message.chat.id}_{bot_script_name}',
                          emoji_key='cross', style="danger")
        markup.add(stop_button)
        text = (
            f'{ce("check")} <b>تم رفع الملف بنجاح</b>\n'
            f'{ce("python")} <code>{bot_script_name}</code>\n\n'
            f'{ce("bell")} إذا طلب الملف أي بيانات (رقم، كود، كلمة سر) سيتم إرسالها لك هنا.'
        )
        try:
