...
def main_menu(user=None):
    ...
    markup.add(
        InlineKeyboardButton(
            "اضفني الى مجموعتك +",
            url=(
                f"http://t.me/{bot.get_me().username}"
                f"?startgroup=true"
            ),
            style="primary"
        )
...
def check_group_messages(message):
    ...
    if is_admin_or_creator:
        ...
        if text_content == "تعطيل":
            ...
            bot.reply_to(
                message,
                "<b>تم تعطيل البوت وإيقاف الاشتراك الإجباري "
                "لهذه المجموعة بنجاح</b>",
                parse_mode="HTML"
            )
...
# تم تعديل سطر التشغيل هنا لضمان استقرار الاتصال على السيرفر
